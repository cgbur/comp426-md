...
7. `onLose(callback)`: Takes a callback function as input and registers that function as a listener to the `love` event. When the game transitions into a state where no more valid moves can be made, the game should call all previously registered `lose` callbacks, passing in the game's current `gameState` as an argument to the function.
