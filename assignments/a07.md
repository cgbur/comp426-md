...
After the assignment due date, an LA will manually load your 2048 game page into a Google Chrome window and will try to play the game. Your user interface doesn't need to be perfect and there don't need to be animations, but the game must be playable for full credit. In particular, the following elements must be present in your user interface:
