...
Inside of `function_basics.js` we are going to define functions that return basic information. Remember to keep the function names the same and to use the `export` keyword so the autograder can inport them. 

Here is the MDN section on [functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Functions). Reading this isn't required but will give you the tools and knowledge to complete the assignment in regards to functions. The later section on advanced functions will have problems that are already solved by the language, so knowing what JavaScript can do is very helpful.
...
Inside each file there will be JSDoc style comments that describe the features of the function.
