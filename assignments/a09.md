...
---------------------------------------------------------------------------------
   body   |  string  | "This is    | The body text of the tweet
          ...
          |          |  user said" |
---------------------------------------------------------------------------------
  author  |   User   |             | A json object with information about the
          |          |             | author of the tweet, including their name
---------------------------------------------------------------------------------
...
Request Params:
	...
	where (json) - Optional. Defaults to an empty object ({}). Allows the requester
	               to specify a filter to apply to the records before retrieving
	               them.
Response:
...
*At an absolute minimum*, your app must allow users to:
...
* For tweets that were posted by the logged-in user, include a button that allows the user to delete their tweet. When the user clicks this button, use the Destroy route (see Section 2.5) to save the user's action to the server.

* Provide a button that allows the user to post a new tweet. When the user clicks this button, create and display an edit form with a `<textarea>` field to receive the tweet body text. After the user finishes composing the tweet, use the Create route (see Section 2.2) to save the new tweet on the server.
...
* Add support for [infinite scrolling](https://en.wiktionary.org/wiki/infinite_scroll)