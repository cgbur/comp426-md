...
So first we are going to define our function. Notice how we no longer need the callback parameter. It might look odd that we are returning something straight away. Just remember that when you call this function you are given back a promise. The promise is that it will eventually give you back a value or an error at some point in the future. That is why we use the `.then()` and `.catch()`. 
