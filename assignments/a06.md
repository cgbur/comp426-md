...
Now we are going to write some code that demonstrates the functionality of callbacks, promises, and async/await. Head over do you workspace folder. Inside `a06/exercises`, fill out each file to complete the task described.
...
### 2.1 SetTimeout
Because we aren't interacting with a server or anything that takes time to respond, we need to simulate an instance where we need asyncronous execution. [SetTimeout()](https://developer.mozilla.org/en-US/docs/Web/API/WindowOrWorkerGlobalScope/setTimeout) is used to execute a block of code after a certain amount of delay. 

You will use this function to simulate a delay of 1.5 seconds (1500 ms) in the assignment. 
- In a callback you can simply use `setTimeout()` and in the function, use the callback.
- In the promises section you should immediately return the promise, and then the promise that was returned should use the `setTimeout()` to delay its resolve by 1.5 seconds.
...
You should have covered promises by this point in class. If you haven't (doing the assignment early or were so incredibly ill you missed class) then the link below should be enough to bring you up to speed on how they work.

See [Promise Documenation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise). The exercises will require that you make and return your own promises.
