...
4. Go back to Visual Studio Code and open the file `a06/1.synchronous/controller.js`. You'll notice a `TODO` comment marking the point where you need to add the moves to solve the maze.
...
1. In Visual Studio Code, open the file `a06/2.asynchronous/controller.js`. You'll notice a `TODO` comment marking the point where you need to add the asynchronous moves to solve the maze.
...
Copy this code into `a06\3.eventhandler\controller.js` directly below the `TODO` comment, save the file, and navigate to the "Event handlers" link in Google Chrome.
...
> Both Section 3 and Section 4 work in exactly the same way. The only difference is the way in which we hand off the "move end" callback function to the token. In Section 3, we store the callback function in the `onmoveend` property; in Section 4, we register it as a listener via the `on()` method. Both of these syntactic approaches are commonly seen in JavaScript. Which one to use depends on the library you're using.

Copy the above code into `a06\4.eventlistener\controller.js` directly below the `TODO` comment, save the file, and navigate to the "Event listeners" link in Google Chrome. You should again see the token successfully navigate the maze. Refresh the browser window if you want to see it again.
