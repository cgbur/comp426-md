...
4. Go back to Visual Studio Code and open the file `a06/synchronous/controller.js`. You'll notice a `TODO` comment marking the point where you need to add the moves to solve the maze.
...
When you call a synchronous function, you are hogging the browser's only JavaScript thread until your code completely finishes running. That means the browser doesn't even get computation time to update the DOM until after your code finishes. Since each token move takes about two seconds and there are four moves in total, this means your code is monopolizing the JavaScript execution thread for about eight consecutive seconds. During this time, the browser can't update how the page looks and it therefore appears blank and locked up. As soon as your code finishes making its moves, the browser gets access to the execution thread and updates the page. But by this time, the moves are already complete and the token is at the final position!

The main takeaway of the synchronous approach is that it allows you to write the most straightforward, clean code, but it comes with the major limitation of locking up the active thread until the process is complete.
...
In addition to the `moveSync()` method that we used in the last section, the `Token` class also exposes a `moveAsync()` method. When `moveAsync()` is called, the token kicks off a new move action just like before. The difference is that instead of "hanging" until the move is complete (which is what the `moveSync()` method did), the `moveAsync()` method *immediately returns back to your code*, even before the move action has completed. This gives you, as the programmer, the flexibility to decide what should happen in the meantime while the move is taking place.

Let's try using the `moveAsync()` function.

1. In Visual Studio Code, open the file `a06/asynchronous/controller.js`. You'll notice a `TODO` comment marking the point where you need to add the asynchronous moves to solve the maze.
...
Google Chrome provides a way to see all JavaScript errors that occur while your web page is active. They appear in a special window called the JavaScript console. Here's how to see it:
...
Copy this code into `a06\eventhandler\controller.js` directly below the `TODO` comment, save the file, and navigate to the "Event handlers" link in Google Chrome.
...
In fact, many existing JavaScript libraries, including jQuery, also take this approach. The standard syntax is for an observable object to expose an `on(event, callback)` function, which allows the programmer to specify (1) which event they want to listen to and (2) register an observer callback function for that event.
...
Here we are registering the same exact function as we did in Section 3, and we are listening to the exact same event that we did in Section 3 (`"moveend"`). The only difference is that we are using the `on()` function, which adds the function to an internal array of listeners stored inside the observable token object.

> Both Section 3 and Section 4 work in exactly the same way. The only difference is the way in which we hand off the "move end" callback function to the token. In Section 3, we stored the callback function in the `onmoveend` property; in Section 4, we register it as a listener via the `on()` method. Both of these syntatical approaches are common in JavaScript.

Copy the above code into `a06\eventlistener\controller.js` directly below the `TODO` comment, save the file, and navigate to the "Event listeners" link in Google Chrome. You should again see the token successfully navigate the maze. Refresh the browser window if you want to see it again.
